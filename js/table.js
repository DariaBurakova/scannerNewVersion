...
<div class="modal-wrap">
  <div class="modal-win" >
      ...
      <div class="col-md-4 m-1" style="margin: 0 auto;">
      <select class="form-select select_option form-select-sm" aria-label="Пример выбора по умолчанию"
                                            id="selectedChooseError">
                                            <option value="0" selected="selected">Выберите причину несоответсвия<</option>
                                            <option value="1">ПКИ</option>
      ...
      <div class="text-end " >
        ...
        <button type="button" class="btn btn-secondary " onclick="handlerControlStatus('${BoxSerial}', '${SkusSerial}', '${OperationNumber}','${textStatus}','${WcGuid}' ,'${parseInt(num)}', '${InOperationNumber}', '${Status}'),deleteElems(document.querySelectorAll('.modal-wrap'))">Да</button>
      </div>
  </div>
</div>
    `)
...
const handlerControlStatus = (BoxSerial, SkusSerial, OperationNumber, textStatus, WcGuid, num, InOperationNumber, Status) => {
    ...
    if (controlStatus > 0) {
        ...
        if (num != 0) {
            ...
            handlerControlMapTimeAndButton(BoxSerial, SkusSerial, OperationNumber, textStatus, WcGuid, parseInt(num), InOperationNumber, checkBoxItemSkusSerial.length)
